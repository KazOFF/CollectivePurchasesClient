...
class ParserItemIndexPage extends GetView<ParserItemIndexPageController> {
  @override
  Widget build(BuildContext context) {
    return Obx(() => AppScaffold(
        title: Text(controller.parserCategory.name ?? ""),
  ...
  Widget _buildBody() {
    ...
    } else if (controller.parserItemList.isEmpty) {
      return EmptyErrorWidget(onPressed: () => controller.refreshList());
    } else {
      return RefreshIndicator(
        onRefresh: controller.refreshList,
        child: GridView.builder(
          itemCount: controller.parserItemList.length,
  ...
  Widget? _buildGridItem(BuildContext context, int index) {
    ParserItemDTO item = controller.parserItemList[index];
    return Card(
    ...
    return controller.isSelectionMode.value ? SpeedDial(
      speedDialChildren: <SpeedDialChild>[
        ...
        SpeedDialChild(
          ...
          onPressed: controller.batchTransferTapped,
        ),
      ],
